...
function __flexlm() {
	...
	if [[ -n "${OP}" && -n "${VLIC}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$FLEXLM_TOOL"
		__progress_bar PB_STRUCTURE
		declare -A config_flexlm=()
		__load_conf "$FLEXLM_CFG" config_flexlm
		STATUS_CONF=$?
		declare -A config_flexlm_util=()
		__load_util_conf "$FLEXLM_UTIL_CFG" config_flexlm_util
		STATUS_CONF_UTIL=$?
		...
		if [ $STATUS -eq $NOT_SUCCESS ]; then
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$FLEXLM_TOOL"
			exit 129
		fi
		...
		if [ $STATUS -eq $SUCCESS ]; then
			...
			local LIC_FILE=$(__get_item "$LFILE" license_list)
			local LIC_PORT=$(__get_item "$LPORT" license_list)
			local LOG_FILE=$(__get_item "$LLOG" license_list)
